  ...
  try {
    ...
    for (const [serviceId, service] of Object.entries(services)) {
      ...
      if (kongServiceCheck.status === 404) {
        const createRes = await fetch(`${KONG_ADMIN_URL}/services`, {
          method: 'POST',
        ...
        if (createRes.ok) {
          ...
          console.warn(`Failed to add service: ${serviceName}`, await createRes.text());
          continue; // Skip to next service
        }
      } else {
        const updateRes = await fetch(`${KONG_ADMIN_URL}/services/${serviceName}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: serviceName,
        ...
        if (updateRes.ok) {
          ...
          console.warn(`Failed to update service: ${serviceName}`, await updateRes.text());
          continue;
        }
      }
      ...
      if (routes && Array.isArray(routes.data)) {
        const existingRoute = routes.data.find(route => route.paths.includes(`/${serviceName}`));
        if (!existingRoute) {
          ...
          if (JSON.stringify(existingRoute.paths) !== JSON.stringify([`/${serviceName}`])) {
            await fetch(`${KONG_ADMIN_URL}/routes/${routeId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                paths: [`/${serviceName}`]
            ...
            console.log(`Updated route: /${serviceName}`);
          }
        }
      } else {
        console.warn(`Unexpected route response for service ${serviceName}:`, routes);
      }
    }
  } catch (err) {
